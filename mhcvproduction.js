...
function cleanData() {
    rawData = rawData.filter(row => row["Region"] && row["Bus/Truck"]);
...
function updateShareChart() {
    ...
    filteredData.forEach(row => {
        const segment = row["Regional Segment"] || "Unknown";
        segmentMap[segment] = (segmentMap[segment] || 0) + toNumber(row[col]);
    });

    const sorted = Object.entries(segmentMap)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8);
...
function updateBrandChart() {
    ...
    filteredData.forEach(row => {
        const brand = row["Production Brand"] || "Unknown";
        brandMap[brand] = (brandMap[brand] || 0) + toNumber(row[col]);
    });
...
function updateDetailsTable() {
    ...
    const sortedRows = [...filteredData]
        .sort((a, b) => toNumber(b[col]) - toNumber(a[col]))
        .slice(0, 100);
    ...
    sortedRows.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td>${safe(row["Country/Territory"])}</td>
            <td>${safe(row["Production Brand"])}</td>
            <td>${safe(row["Sales Parent"])}</td>
            <td>${safe(row["Bus/Truck"])}</td>
            <td>${safe(row["Global Production Segment"])}</td>
            <td>${safe(row["Regional Segment"])}</td>
            <td>${formatNumber(toNumber(row[col]))}</td>
        `;
