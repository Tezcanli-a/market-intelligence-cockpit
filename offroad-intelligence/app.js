...
function v173_esc(x){return v173_txt(x).replace(/[&<>"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[m]))}
function v173_val(o,keys,fb=''){for(const k of keys){if(o&&o[k]!==undefined&&o[k]!==null&&o[k]!=='')return o[k]}return fb}
function v173_tag(x){return `<span class="v173-tag">${v173_esc(x)}</span>`}
function v173_set(id,html){const el=document.getElementById(id);if(el)el.innerHTML=html}
function v173_evidenceForSignal(id){return (state.data.evidence||[]).filter(e=>(e.linkedSignalIds||[]).includes(id)||e.linkedSignalId===id)}
function v173_assessmentForSignal(id){return (state.data.assessments||[]).find(a=>a.signalId===id)}
function v173_corpus(){const d=state.data||{};const blocks=[];v173_arr(d.newsRaw&&d.newsRaw.news).forEach(n=>blocks.push(`${n.company||''} ${n.headline||''} ${n.summary||''} ${n.category||''}`));['signals','assessments','customerProfiles','competitorProfiles','technologies','opportunities','risks','patentSignals','hiringSignals','technicalPublications'].forEach(k=>v173_arr(d[k]).forEach(x=>blocks.push(JSON.stringify(x))));return v173_norm(blocks.join(' '))}
function v173_count(names,corpus){return names.map(name=>{const n=v173_norm(name);if(!n)return{name,count:0};const re=new RegExp(n.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'g');return{name,count:(corpus.match(re)||[]).length}}).filter(x=>x.count>0).sort((a,b)=>b.count-a.count)}
function v173_bars(items){const max=Math.max(1,...items.map(x=>x.count));return items.map(x=>`<div class="v173-bar-row"><strong>${v173_esc(x.name)}</strong><div class="v173-track"><div class="v173-fill" style="width:${Math.max(5,(x.count/max)*100)}%"></div></div><span>${x.count}</span></div>`).join('')}
function v173_cloud(items){const max=Math.max(1,...items.map(x=>x.count));return items.map(x=>{const size=14+Math.round((x.count/max)*18);const cls=x.count>=max*.75?'hot':x.count>=max*.45?'up':'flat';return`<span class="${cls}" style="font-size:${size}px">${v173_esc(x.name)} ${x.count}</span>`}).join('')}
function v173_priorityLabel(item){const p=v173_norm(v173_val(item,['priority','riskLevel','confidence'],''));if(p.includes('high'))return'High';if(p.includes('medium'))return'Medium';if(p.includes('low'))return'Low';return'Medium'}
function v173_card(title,meta,body,type='neutral'){return `<article class="v173-card ${type}"><div class="v173-card-meta">${v173_esc(meta)}</div><h3>${v173_esc(title)}</h3><p>${v173_esc(body)}</p></article>`}
function v173_profileOpportunityScore(profile){const d=profile.level3DynamicIntelligence||{};return v173_arr(d.opportunities).length+(d.accountOpportunity?1:0)+v173_arr(d.technologyThemes).length}
function v173_strategicAccounts(){return (state.data.customerProfiles||[]).map(p=>({name:p.name||p.customer,score:v173_profileOpportunityScore(p),themes:v173_arr(p.level3DynamicIntelligence&&p.level3DynamicIntelligence.technologyThemes),actions:v173_arr(p.level3DynamicIntelligence&&p.level3DynamicIntelligence.nextActions)})).sort((a,b)=>b.score-a.score).slice(0,5)}
function renderOverview(signals){
  const useSignals=(signals&&signals.length?signals:(state.data.signals||[]));
  const assessments=state.data.assessments||[];
  const opportunities=state.data.opportunities||[];
  const risks=state.data.risks||[];
  const patents=state.data.patentSignals||[];
  const hiring=state.data.hiringSignals||[];
  const publications=state.data.technicalPublications||[];
  const corpus=v173_corpus();
  const techNames=[...new Set(['Automation','Operator Environment','Integrated Cabin','HMI','Sensors','Electrification','Precision Agriculture','Autonomy','Sustainable Materials','Smart Cockpit',...(state.data.technologies||[]).map(t=>t.theme||t.name).filter(Boolean)])];
  const compNames=[...new Set((state.data.competitorProfiles||[]).map(c=>c.name||c.competitor).filter(Boolean))];
  const custNames=[...new Set((state.data.customerProfiles||[]).map(c=>c.name||c.customer).filter(Boolean))];
  const tech=v173_count(techNames,corpus).slice(0,10);
  const comp=v173_count(compNames,corpus).slice(0,6);
  const cust=v173_count(custNames,corpus).slice(0,6);

  const actNow=[];
  opportunities.filter(o=>v173_priorityLabel(o)==='High').slice(0,3).forEach(o=>actNow.push(v173_card(v173_val(o,['opportunity','title','name'],'Opportunity'), 'Opportunity · High priority', v173_val(o,['nextAction','whyAttractive','status'],'Define next action'), 'opportunity')));
  risks.filter(r=>v173_priorityLabel(r)==='High').slice(0,3).forEach(r=>actNow.push(v173_card(v173_val(r,['riskDescription','description','title','name'],'Risk'), 'Risk · High priority', v173_val(r,['mitigation','potentialImpact','status'],'Define mitigation'), 'risk')));
  assessments.filter(a=>a.confidence==='High'||a.confidence==='Medium').slice(0,3).forEach(a=>actNow.push(v173_card(a.title, `${a.assessmentId||''} · ${a.confidence||'No confidence'}`, a.businessImplication||a.forecast||'No implication populated', 'assessment')));

  const confidenceCounts={High:assessments.filter(a=>a.confidence==='High').length,Medium:assessments.filter(a=>a.confidence==='Medium').length,Low:assessments.filter(a=>a.confidence==='Low').length};
  const singleSource=assessments.filter(a=>v173_evidenceForSignal(a.signalId).length<=1).length;
  const multiSource=assessments.filter(a=>v173_evidenceForSignal(a.signalId).length>=2).length;

  v173_set('overviewSignalsGrid', `
    <div class="v173-overview-full">
      <div class="v173-panel-title"><span>Executive attention</span><h2>Act Now</h2><small>High-priority opportunities, risks and assessments.</small></div>
      <div class="v173-actnow-grid">${actNow.length?actNow.join(''):'<p class="empty">No act-now items yet.</p>'}</div>
    </div>`);

  v173_set('overviewAssessmentsGrid', `
    <article class="v173-panel"><h3>Momentum intelligence</h3><div class="v173-cloud">${tech.length?v173_cloud(tech):'<span>No technology mentions yet</span>'}</div></article>
    <article class="v173-panel"><h3>Most visible competitors</h3>${comp.length?v173_bars(comp):'<p class="empty">No competitor mentions yet.</p>'}</article>
    <article class="v173-panel"><h3>Most discussed customers</h3>${cust.length?v173_bars(cust):'<p class="empty">No customer mentions yet.</p>'}</article>
    <article class="v173-panel"><h3>Confidence dashboard</h3><div class="v173-confidence-grid"><div><b>${confidenceCounts.High}</b><span>High</span></div><div><b>${confidenceCounts.Medium}</b><span>Medium</span></div><div><b>${confidenceCounts.Low}</b><span>Low</span></div><div><b>${singleSource}</b><span>Single-source</span></div><div><b>${multiSource}</b><span>2+ evidence</span></div></div><p class="v173-note">Keep this visible to avoid polished UI being over-read as decision-grade certainty.</p></article>`);

  const accounts=v173_strategicAccounts();
  v173_set('overviewOpportunities', accounts.map(a=>`<div class="v173-mini-row"><strong>${v173_esc(a.name)}</strong><span>Score ${a.score}</span><p>${a.themes.slice(0,3).map(v173_tag).join('')}</p><small>${v173_esc(a.actions[0]||'Define account next action')}</small></div>`).join('')||'<p class="empty">No strategic accounts yet.</p>');
  v173_set('overviewRisks', `
    <div class="v173-mini-row"><strong>Technology Watch</strong><span>Patent-ready</span><p>${v173_tag('Patent signals '+patents.length)}${v173_tag('Hiring signals '+hiring.length)}${v173_tag('Tech publications '+publications.length)}</p><small>Designed for IP7 monthly patent overview integration.</small></div>
    <div class="v173-mini-row"><strong>Evidence quality gate</strong><span>V18.1-ready</span><p>${v173_tag('2+ independent sources before High confidence')}</p><small>Assessment confidence should depend on source diversity, not UI polish.</small></div>`);
}
